...
@click.option("-k", "--topk", type=int, default=3)
@click.option("-s", "--stride", type=int, default=1)
@click.option("-b", "--n-batches", type=int, default=32)
@click.option("-o", "--output-dir", type=str, default="./results")
@click.option("--cuda/--cpu", default=True)
def demo4(image_paths, target_layer, arch, topk, stride, n_batches, output_dir, cuda):
    """
    Visualize model responses given multiple images
    """
    ...
    #for i in range(topk): #original code
    for i in range(1):
        for p in patche_sizes:
            print("Patch:", p)
            sensitivity = occlusion_sensitivity(
...
@click.option("-i", "--image-paths", type=str, multiple=True, required=True)
@click.option("-a", "--arch", type=click.Choice(model_names), required=True)
@click.option("-t", "--target-layer", type=str, multiple=True, required=True)
@click.option("-k", "--topk", type=int, default=5)
@click.option("-o", "--output-dir", type=str, default="./results")
@click.option("--cuda/--cpu", default=True)
def demo5(image_paths, target_layer, arch, topk,  output_dir, cuda): 
    ##
    ##
    ####Pendiente de terminar, nunca ha corrido
    ##
    ##
    """
    Visualize model responses given multiple images
    ...
    for i in range(topk):
        ...
        gcam.backward(ids=ids[:, [i]])
      ######################################################################################
      #for target_layer in target_layers:   #esta pendiente añadir este FOR <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<----------------------------------------------
        regions = gcam.generate(target_layer=target_layer)
        for j in range(len(images)):
            print("\t#{}: {} ({:.5f})".format(j, classes[ids[j, i]], probs[j, i]))
            # Guided Backpropagation
...
@click.option("-o", "--output-dir", type=str, default="./results")
@click.option("--cuda/--cpu", default=True)
def demo6(image_paths, output_dir, cuda):
    """
    Generate Grad-CAM at different layers of ResNet-152
    """
    device = get_device(cuda)
